...
**优化方案**：
\`\`\`rust
// src/core/segments/byebyecode_usage.rs:152-159
...
// 状态色逻辑
let (icon, color) = match percentage {
    0.0..=50.0 => ("✓", AnsiColor::Color16 { c16: 10 }),  // 绿色 - 充足
    50.0..=80.0 => ("⚠", AnsiColor::Color16 { c16: 11 }), // 黄色 - 注意
    _ => ("⚠", AnsiColor::Color16 { c16: 9 }),            // 红色 - 紧急
};
\`\`\`

**预期收益**：
- 关键信息（百分比）放首位，扫视速度提升 50%
- 状态色直观识别风险，无需阅读数字
...
**当前问题**：
\`\`\`
❌ FREE ¥88/年 可重置2次 28天 | PAYGO ¥66/年 989天 | PLUS ¥198/月 可重置2次 53天
\`\`\`
- 多套餐时横向爆炸（80+ 字符）
- 动态随机色无语义（用户无法通过颜色识别套餐类型）
- 快过期套餐淹没在中间

**优化方案**：
\`\`\`rust
// src/core/segments/byebyecode_subscription.rs:135-156

// 精简模式（默认）
👑 PLUS 53d | 💳 PAYGO 989d | ⚠️ FREE 28d
// 减少50%字符，突出天数

// 固定语义色方案
let color = match sub.plan_name.as_str() {
    "PLUS" | "PRO" | "MAX" => AnsiColor::Color256 { c256: 214 }, // 橙色 - 高级套餐
    "PAYGO" => AnsiColor::Color256 { c256: 39 },                 // 蓝色 - 按需套餐
    "FREE" => AnsiColor::Color16 { c16: 8 },                     // 灰色 - 免费套餐
    _ => AnsiColor::Color16 { c16: 7 },                          // 白色 - 默认
};
\`\`\`

**预期收益**：
- 字符数减少 50-60%，窄终端友好
- 固定语义色，用户无需阅读文字即可识别套餐类型
